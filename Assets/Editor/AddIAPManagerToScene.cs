...
namespace RingGameEditor
{
    ...
    /// kèm menu item để thêm thủ công. Port từ moneyrace-skybranch/TapTapGame.
    /// </summary>
    public static class AddIAPManagerToScene
    {
        // Menu.unity là scene đầu tiên load (xem EditorBuildSettings) — IAPManager
        // là DontDestroyOnLoad nên chỉ cần có mặt ở đây là sống xuyên suốt sang Game.unity.
        ...
        [MenuItem("Tools/RingGame/Add IAPManager to Gameplay Scene")]
        public static void AddIAPManagerToGameplay()
        {
            AddIAPManagerToSceneAtPath(GameplayScenePath);
        }

        [MenuItem("Tools/RingGame/Add IAPManager to Current Scene")]
        public static void AddIAPManagerToCurrentScene()
        {
            var scene = UnityEngine.SceneManagement.SceneManager.GetActiveScene();
            if (!scene.IsValid() || string.IsNullOrEmpty(scene.path))
            {
                EditorUtility.DisplayDialog("Add IAPManager", "Mở một scene trước (ví dụ: Menu.unity).", "OK");
                return;
            }
        ...
        /// <summary>Dò cả trong con, không chỉ root (dễ tạo trùng nếu chỉ dò root).</summary>
