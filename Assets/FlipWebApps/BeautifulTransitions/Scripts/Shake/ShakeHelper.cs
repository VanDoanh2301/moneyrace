...
namespace BeautifulTransitions.Scripts.Shake
{
    ...
    {
        static readonly List<EntityId> ActiveShakes = new List<EntityId>(2);
        ...
        static IEnumerator ShakeCoroutine(Transform transform, float duration, Vector3 range, float decayStart = 1f)
        {
            ...
            if (!ActiveShakes.Contains(transformId))
            {
                ActiveShakes.Add(transformId);
                ...
                ActiveShakes.Remove(transformId);
            }
        }
    }
}