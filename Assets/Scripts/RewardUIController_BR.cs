...
using UnityEngine.UI;
    ...
    public class RewardUIController_BR : MonoBehaviour
    {
        ...
        IEnumerator CRPlayRewardAnim(int rewardValue)
        {
            ...
            for (int i = 1; i <= rewardValue; i++)
            {
                ...
                SoundManager_BR.Instance.PlaySound(SoundManager_BR.Instance.tick);
                yield return new WaitForSeconds(0.03f);
            }

            // Actually store the rewards.
            CoinManager_BR.Instance.AddCoins(rewardValue);

            yield return new WaitForSeconds(0.2f);

            SoundManager_BR.Instance.PlaySound(SoundManager_BR.Instance.rewarded);
