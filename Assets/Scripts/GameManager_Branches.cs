...
public class GameManager_Branches : MonoBehaviour
{
    public static GameManager_Branches Instance { get; private set; }
    public static event System.Action<GameState_BR, GameState_BR> GameState_BRChanged = delegate { };

    public GameState_BR GameState_BR
    {
        get
        {
            return _GameState_BR;
        }
        private set
        {
            if (value != _GameState_BR)
            {
                GameState_BR oldState = _GameState_BR;
                _GameState_BR = value;

                GameState_BRChanged(_GameState_BR, oldState);
            }
        }
    }

    [SerializeField]
    private GameState_BR _GameState_BR = GameState_BR.Prepare;
    ...
    public PlayerController_Branches playerController;
    ...
    void OnEnable()
    {
        PlayerController_Branches.PlayerDie += PlayerController_PlayerDie;
    }
    ...
    void Start()
    {
        GameState_BR = GameState_BR.Prepare;
    ...
    {
        GameState_BR = GameState_BR.Playing;
    }

    public void GameOver()
    {
        SoundManager_BR.Instance.PlaySound(SoundManager_BR.Instance.gameOver);
        GameCount++;
        GameState_BR = GameState_BR.GameOver;
    }
    ...
    IEnumerator RandomTrunkAndBranch()
    {
        ...
        {
            yield return null;
            if (GameState_BR != GameState_BR.GameOver && playerController.isPlayerRunning && !playerController.isRotatingTrunk && !playerController.touchDisable)
            {
    ...
    IEnumerator MoveAllTrunk()
    {
        ...
        {           
            ...
            {               
                if (GameState_BR != GameState_BR.GameOver)
                {
